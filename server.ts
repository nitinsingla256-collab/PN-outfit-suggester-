...
`;
  ...
  app.post('/api/gemini/chat', authMiddleware, async (req, res) => {
    try {
      ...
      const systemPrompt = `You are PAURVI Concierge — an elite personal fashion stylist and wardrobe archivist for client ${req.user!.name}.
...
${
  userWardrobe.length === 0
    ...
    : `Total catalogued pieces: ${userWardrobe.length}. Items: ` +
      userWardrobe.map((i: any) => `"${i.name}" (${i.category}, ${i.color}, ${i.fit || 'Tailored'}, worn ${i.timesWorn || 0} times)`).join('; ')
}

Saved looks in lookbook: ${userOutfits.length}.

STRICT ACCURACY:
- Only reference garments that actually exist in the client's wardrobe when proposing combinations.
- If the user asks questions like "What should I wear today?", "What can I wear with my black trousers?", "Create 3 outfits from my wardrobe", "Build an outfit around this shirt", formulate creative combinations using their actual inventory.
- If an essential item is missing, mention it honestly and recommend how to style around it.`;

      const contents: any[] = [];
      contents.push({ role: 'user', parts: [{ text: systemPrompt }] });
      contents.push({ role: 'model', parts: [{ text: `Understood. I am ready to advise ${req.user!.name} with sartorial mastery.` }] });
